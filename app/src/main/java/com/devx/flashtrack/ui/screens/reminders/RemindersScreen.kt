...
fun RemindersScreen(viewModel: MainViewModel, onBack: () -> Unit) {
    ...
    val list     = if (selectedTab == 0) active else inactive
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            if (list.isEmpty()) {
                item {
                    EmptyState("🔔", "No reminders", "Add reminders for bills,\nsubscriptions & recurring payments")
                }
            } else {
    ...
    if (showAddSheet) {
        AddReminderSheet(onDismiss = { showAddSheet = false }, onSave = { r ->
            viewModel.addReminder(r)
            showAddSheet = false
        })
    }
}
...
private fun ReminderCard(
    ...
    val dueDate = SimpleDateFormat("dd MMM yyyy", Locale.getDefault()).format(Date(reminder.nextDueDate))
    ...
    val accentColor = if (!reminder.isActive) MaterialTheme.colorScheme.onSurfaceVariant
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(2.dp)) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text("📅 $dueDate", style = MaterialTheme.typography.labelSmall, color = accentColor)
                    Text("🔁 $intervalLabel", style = MaterialTheme.typography.labelSmall,
                         color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
                if (reminder.amount > 0) {
                    Text("₹${"%,.0f".format(reminder.amount)}",
            ...
            Column(horizontalAlignment = Alignment.End, verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Switch(
                    ...
                    colors = SwitchDefaults.colors(
                        checkedThumbColor = OrangeExpense,
                        checkedTrackColor = OrangeExpense.copy(alpha = 0.3f)
                    )
                )
...
private fun AddReminderSheet(onDismiss: () -> Unit, onSave: (ReminderEntity) -> Unit) {
    ...
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(
            ...
            androidx.compose.foundation.lazy.LazyRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                items(ReminderInterval.values()) { iv ->
                    FilterChip(
                        selected = interval == iv,
                        onClick = { interval = iv },
                        label = { Text(iv.name.lowercase().replaceFirstChar { it.uppercase() },
                                       style = MaterialTheme.typography.labelSmall) },
                        colors = FilterChipDefaults.filterChipColors(
                            selectedContainerColor = OrangeExpense.copy(alpha = 0.2f),
                            selectedLabelColor = OrangeExpense
                        )
                    )
                }
            }
            ...
            OutlinedButton(
                ...
                Text("📅 Due: ${java.text.SimpleDateFormat("dd MMM yyyy", java.util.Locale.getDefault()).format(java.util.Date(selectedDate))}")
            }

            Button(
                onClick = {
                    if (title.isBlank()) return@Button
                    onSave(ReminderEntity(
                        title = title,
    ...
    if (showDatePicker) {
        ...
        DatePickerDialog(
            ...
            confirmButton = {
                TextButton(onClick = { state.selectedDateMillis?.let { selectedDate = it }; showDatePicker = false }) {
                    Text("OK")
                }
            }
