        ...
        LazyColumn(
            ...
            item {
                Row(
                    ...
                    DebtSummaryCard("You'll Receive", totalLent ?: 0.0, GreenIncome, "🤝", Modifier.weight(1f))
                    DebtSummaryCard("You Owe", totalBorrowed ?: 0.0, RedExpense, "💸", Modifier.weight(1f))
                }
            }
            ...
            item {
                Row(
                    ...
                    listOf("Lent Out (${lent.size})", "Borrowed (${borrowed.size})", "Settled (${settled.size})").forEachIndexed { i, label ->
                        Box(
                            modifier = Modifier.weight(1f)
                                .clip(RoundedCornerShape(12.dp))
                                .background(if (selectedTab == i) PurpleDebt.copy(alpha = 0.8f) else Color.Transparent)
            ...
            val list = when (selectedTab) { 0 -> lent; 1 -> borrowed; else -> settled }

            if (list.isEmpty()) {
                item {
                    EmptyState(
                        if (selectedTab == 2) "✅" else "🤝",
                        if (selectedTab == 0) "No money lent" else if (selectedTab == 1) "No money borrowed" else "Nothing settled yet",
                        "Tap + to add an IOU entry"
                    )
                }
            } else {
...
@Composable
private fun DebtSummaryCard(label: String, amount: Double, color: Color, emoji: String, modifier: Modifier = Modifier) {
    Box(
        modifier = modifier
...
private fun DebtCard(debt: DebtEntity, onSettle: () -> Unit, onDelete: () -> Unit) {
    ...
    val progress = if (debt.amount > 0) (debt.settledAmount / debt.amount).toFloat() else 0f
    ...
    Card(
        ...
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Row(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Column {
                        ...
                        Text(
                            if (debt.type == DebtType.LENT) "You lent" else "You borrowed",
                            style = MaterialTheme.typography.labelSmall,
            ...
            if (debt.notes.isNotBlank()) {
                Text(debt.notes, style = MaterialTheme.typography.bodySmall,
                     color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            ...
            if (debt.settledAmount > 0) {
                Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                    Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        Text("Settled: ₹${"%,.0f".format(debt.settledAmount)}",
                             style = MaterialTheme.typography.labelSmall, color = GreenIncome)
                    ...
                    LinearProgressIndicator(
                        ...
                        color = GreenIncome,
                        trackColor = GreenIncome.copy(alpha = 0.15f)
                    )
                }
            }
        ...
        Column(
            ...
            Row(
                ...
                listOf(DebtType.LENT to "I Lent", DebtType.BORROWED to "I Borrowed").forEach { (t, label) ->
                    Box(
                        modifier = Modifier.weight(1f)
                            .clip(RoundedCornerShape(12.dp))
                            .background(if (type == t) PurpleDebt.copy(alpha = 0.8f) else Color.Transparent)
                            .clickable { type = t }
            ...
            Button(
                onClick = {
                    ...
                    if (personName.isBlank() || amt <= 0) return@Button
...
private fun SettleDialog(debt: DebtEntity, onDismiss: () -> Unit, onSettle: (Double) -> Unit) {
    ...
    AlertDialog(
        ...
        confirmButton = {
            Button(
                onClick = { onSettle(amount.toDoubleOrNull() ?: remaining) },
                colors = ButtonDefaults.buttonColors(containerColor = PurpleDebt)
            ) { Text("Settle") }
        },
